...
class LiveStreamHostApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: LiveStreamHostApi?, messageChannelSuffix: String = "") {
    ...
    let toggleVideoMuteChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.apivideo_live_stream.LiveStreamHostApi.toggleVideoMute\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      toggleVideoMuteChannel.setMessageHandler { _, reply in
        do {
          try api.toggleVideoMute()
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
      ...
      toggleVideoMuteChannel.setMessageHandler(nil)
    }
